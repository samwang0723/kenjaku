...
// Auto-load dev token for local environment if no saved token exists.
// The .dev-token file is generated by `make dev-setup` and gitignored.
if (!savedToken && currentEnv === 'local') {
  fetch('/.dev-token').then(function(r) { return r.ok ? r.text() : ''; }).then(function(t) {
    if (t && bearerTokenInput) {
      bearerTokenInput.value = t.trim();
      localStorage.setItem('bearerToken', t.trim());
    }
  }).catch(function() {});
}
